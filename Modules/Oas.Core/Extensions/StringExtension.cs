...
namespace TDevs.Core
{
    public static class StringExtension
    {
        ...
        public static bool MyIsNumeric(string intIn)
        {
            bool MyIsNumeric = false;
            MyIsNumeric = false;
            if (!string.IsNullOrEmpty(intIn))
            {
                if (IsNumber(intIn))
                {
                    MyIsNumeric = true;
        ...
        public static string AppendComment(string strIn, string strRefDoc, string strHeading)
        {
            string AppendComment = "";
            AppendComment = strIn;
            if ((strRefDoc.Length > 0))
            {
                if ((strIn.Length > 0))
                {
                    AppendComment = AppendComment + "<br/>";
                }
                AppendComment = AppendComment + strHeading + ": " + strRefDoc;
            }
            return AppendComment;
        }


        public static string Getroll(string roll,string frame)
        {
              if (roll.Length > 0 || frame.Length > 0)
                       {
                           roll = "Roll: " + roll + " Frame: " + frame;
                       }
            return roll;
        }
        ...
        public static string ToShortDate(string input)
        {
            string date = string.Empty;
            if (!string.IsNullOrEmpty(input))
            {
                date = Convert.ToDateTime(input).ToShortDateString();
            }
            return date;
        }
    }
